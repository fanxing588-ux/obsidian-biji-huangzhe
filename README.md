...
- **看原文 →**:跳到原笔记,光标定位到那一段
- **有用 / 没用**:反馈,**会影响后续排序**(见下面"个性化飞轮")
- **别再提醒**:把这一条加入隐藏列表

### 个性化飞轮(v0.4.0)

"有用 / 没用"不只是给那一条卡片调权,而是会**算一个"你的偏好向量"**(Rocchio centroid,带 30 天半衰期),给主题相似于你过去喜欢的候选加分:

- 标一条"母婴关系"卡片"有用" → 后续别的母婴主题卡片也排得更前
- 标几条"反向"类卡片"没用" → 后续反向降权,案例 / 金句相对浮多
- 用越久越准。冷启动(0 反馈)时退化成纯检索

设置 → 笔记拾荒者 → **反打扰 → 个性化强度**:0 关闭 / 1 适中(默认推荐) / 2 激进。换 embedding 模型(A↔B 切换)会自动失效重算,不用手动清。
...
- "有用 / 没用"反馈接入排序逻辑(之前的版本只是占位 Notice)
- 新增**偏好向量**(Rocchio centroid,带 30 天半衰期):用户标过的卡片 embedding 聚合成一个个性化向量,给主题相似的候选加分
- per-chunk 权重加大:useful +0.02 → +0.05,useless -0.05 → -0.10(之前太弱不影响排名)
- 新设置项 **个性化强度**:0/1/2,默认 1。设置位置:反打扰区
- 冷启动友好:0 反馈时退化成纯检索;换 embedding 模型时自动失效重算
